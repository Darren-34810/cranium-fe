const Image = ({children, variant, className}) => {
  ...
  const rectangle = "rounded-lg h-32 w-56 bg-black"

  return (
    <div
      className={`${variant === 'rectangle' ? rectangle : (variant === 'square' ? square : (variant === 'small' ? small : (variant === 'medium' ? medium : large)))} ${className ?? ''}`}>{children}</div>
